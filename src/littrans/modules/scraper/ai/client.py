...
from littrans.modules.scraper.config import AI_JITTER


def _make_ai_client() -> genai.Client:
    from littrans.config.settings import settings
    keys = settings.all_gemini_keys
    if not keys:
        raise SystemExit("[ERR scraper] Không tìm thấy GEMINI_API_KEY trong .env")
...
def _get_ai_max_rpm() -> int:
    from littrans.config.settings import settings
    return settings.scraper_ai_max_rpm
...
class AIRateLimiter:
    ...
    def __init__(self, max_rpm: int | None = None) -> None:
        if max_rpm is None:
            max_rpm = _get_ai_max_rpm()
